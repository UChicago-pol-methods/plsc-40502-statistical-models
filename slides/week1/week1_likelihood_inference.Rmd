...
- Logistics:
  - Lectures Tues/Thurs - 9:30am - 10:50pm;
  - 3 Problem Sets (~ 2 weeks)
  - Midterm/Final Exam (~ 1 week)
...
- .maroon[Exams] (65% of your grade)
  - Exams will be structured like the problem sets with two main differences:
    - You have about 1 week to complete them instead of 2
    - You may **not** collaborate with one another on the exams.
--
...
- **Week 9:** TBA (possibly semi-parametric inference)
---
...
$$\widehat{Var(\hat{\beta})} = (\mathbf{X}^{\prime}\mathbf{X})^{-1} \mathbf{X}^{\prime}\hat{\Sigma}\mathbf{X}(\mathbf{X}^{\prime}\mathbf{X})^{-1}$$ where $\hat{\Sigma}$ is our estimate of the variance-covariance matrix using the squared residuals on the diagonals
--
...
$$\frac{\partial^2}{\partial\pi^2}\ell(\pi | \mathbf{y}) = -\frac{1}{\pi^2}\sum_{i=1}^n y_i - \frac{1}{(1-\pi)^2}\sum_{i=1}^n (1-y_i)$$
--
...
$$\mathcal{I}_n = -\mathbb{E}\left[-\frac{1}{\pi^2}\sum_{i=1}^n y_i - \frac{1}{(1-\pi)^2}\sum_{i=1}^n (1-y_i)\right]$$
$$\mathcal{I}_n = \mathbb{E}\left[\frac{1}{\pi^2}\sum_{i=1}^n y_i + \frac{1}{(1-\pi)^2}\sum_{i=1}^n (1-y_i)\right]$$
...
$$\mathcal{I}_n = \frac{1}{\pi^2}\sum_{i=1}^n \mathbb{E}[y_i] + \frac{1}{(1-\pi)^2}\sum_{i=1}^n \mathbb{E}[(1-y_i)]$$
