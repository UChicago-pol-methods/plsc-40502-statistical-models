...
```{r, echo=F}
set.seed(60)
penguins_em_6 <- gmm_em(Y=as.matrix(penguins_complete %>% select(flipper_length_mm, bill_length_mm)), K=6)
```

```{r,warning=F, message=F, echo=F, fig.height=6, fig.width=8, fig.align="center"}
em_clust_df_6 <- as.data.frame(penguins_em_6$mu)
em_clust_df_6$emcluster2 <- as.character(1:6)

penguins_complete$emcluster2 <- as.character(apply(penguins_em_6$gamma, 1, function(x) which.max(x)))
penguins_complete$clusterWeight2 <- apply(penguins_em_6$gamma, 1, function(x) max(x))

penguins_complete %>% ggplot(aes(x=flipper_length_mm, y=bill_length_mm, colour=emcluster2,alpha = clusterWeight2)) + geom_point(size=2.5) + xlab("Flipper Length (mm)") + ylab("Bill Length (mm)") + geom_point(aes(colour=emcluster2, alpha=1), size=10, data=em_clust_df_6, shape=18) + theme_bw() +  theme(text = element_text(size = 20)) + ggtitle(str_c("Gaussian Mixture - 6 Components")) + theme(legend.position = "none")
```
...
- Choosing $K$ is a problem of model comparison and selection.
  ...
  - But we want to avoid **over-fitting**
--
- Just as before, two approaches
...
```{r, echo=F}
gmm_em_manyruns <- function(Y, K, maxit=5000, runs=20, tol=1e-6, verbose=F){
  results <- list()
  likelihoods <- rep(NA, runs)
  for (run in 1:runs){
    D <- ncol(Y) # Number of dimensions
    ## Initialize the parameters (pick some reasonable starting points)
    mu_k <- MASS::mvrnorm(K, colMeans(Y), Sigma = var(Y)) # Matrix of means
    ...
    results[[run]] <- list(mu= mu_k, sigma = sigma_k, pi=pi, gamma=gamma, log_lik = new_lik)
    likelihoods[run] <- new_lik
  }
  return(results[[which.max(likelihoods)]])
}
```

```{r, echo=F, cache=T, warning=F, results=F, message=F}
set.seed(60639)
K_choice <- 2:10
k_likelihoods <- rep(NA, 9)
for (k in K_choice){
  #print(k)
  k_likelihoods[k-1] <- gmm_em_manyruns(Y=as.matrix(penguins_complete %>% select(flipper_length_mm, bill_length_mm)), K=k, runs=10)$log_lik
}
```

```{r,warning=F, message=F, echo=F, fig.height=6, fig.width=8, fig.align="center"}
plot(x=2:10, y=k_likelihoods, type="l", xlab="Number of Clusters", ylab="Log-likelihood")
```
...
- Information criteria results depend on how much you penalize the additional parameters
  - $\text{AIC} = 2k - 2\ell(\hat{\theta})$ shows minimal improvement beyond $6$ clusters (but still potentially some!)

```{r,warning=F, message=F, echo=F, fig.height=6, fig.width=8, fig.align="center"}
plot(x=K_choice, y=2*7*(K_choice) - 2*k_likelihoods, type="l", xlab="Number of Clusters", ylab="AIC")
```
...
```{r, results=F, warning=F, message=F, echo=F, cache=T}
# k-fold cross-validation
# randomly assign data to CV-folds
set.seed(605)
nFolds <- 10
penguins_complete$cv_fold <- sample.int(nFolds, size=nrow(penguins_complete), replace=T)
K_choice_cv <- 2:6
ho_likelihoods <- rep(NA, length(K_choice_cv))
for (k in K_choice_cv){
  heldout_lls <- rep(NA, nFolds)
  for (fold in 1:nFolds){
    fold_model <- gmm_em_manyruns(Y=as.matrix(penguins_complete %>% filter(cv_fold != fold) %>% select(flipper_length_mm, bill_length_mm)), K=k, runs=5)
    heldout_lls[fold] <- gmm_loglik(Y=as.matrix(penguins_complete %>% filter(cv_fold == fold) %>% select(flipper_length_mm, bill_length_mm)), K=k, fold_model$mu, fold_model$sigma, fold_model$pi)/sum(penguins_complete$cv_fold == fold)
  }
  ho_likelihoods[k-1] = mean(heldout_lls)
}
```

```{r,warning=F, message=F, echo=F, fig.height=6, fig.width=8, fig.align="center"}
plot(x=K_choice_cv, y=ho_likelihoods, type="l", xlab="Number of Clusters", ylab="Average Held-Out Log-likelihood")
```
...
- History of topic models
  - **Latent Dirichlet Allocation** $\leadsto$ **Correlated Topic Model** $\leadsto$ **Structural Topic Model**
...
out <- prepDocuments(processed$documents, processed$vocab, processed$meta,
                     lower.thresh=round(.01*nrow(poliblog)), upper.thresh = round(.99*nrow(poliblog)))
```
...
- **Latent Dirichlet Allocation** (Blei, Ng, Jordan, 2003)
  - A generative model for **words** $w$ - defined as unit-vectors from a discrete vocabulary $\{1, 2, \dotsc, V\}$
  - **Documents** are collections of words $\mathbf{w} = \{w_1, w_2, \dotsc w_N\}$
  - A **corpus** is a collection of documents $\{\mathbf{w}_1, \mathbf{w}_2, \dotsc \mathbf{w}_D\}$
--
...
- Two main targets of inference:
  - The **document topic proportions**: $\theta = \{\theta_1, \theta_2, \dotsc \theta_D\}$
  - The **topic-word distributions**: $\beta = \{\beta_1, \beta_2, \dotsc \beta_k\}$
  
---

# Latent Dirichlet Allocation

```{tikz, lda, echo=F, fig.align='center'}
...
\edge{theta}{z};
\edge{alpha}{theta};
\plate [inner sep=.1cm] {plate1} {(w) (z)} {$N$};
\plate [inner sep=.1cm] {plate2} {(plate1) (theta)} {$D$};
\plate [inner sep=.1cm] {plate3} {(beta)} {$K$};
\end{tikzpicture}
```
...
- The **Correlated Topic Model** assumes that each document $\mathbf{w}_d$ is generated by:
  ...
  2. For each of the $N$ words $w_{d,n}$
      - Choose topic $z_{d,n} \sim \text{Multinomial}(f(\eta_d))$ where $f(\eta_{d,k}) = \exp(\eta_{d,k})/\sum_{j} \exp(\eta_{d,j})$
      - Choose word $w_{d,n} \sim \text{Mutinomial}(\beta_{z_{d,n}})$
...
```{tikz, ctm, echo=F, fig.align='center'}
\usetikzlibrary{arrows}
\usetikzlibrary{bayesnet}
\begin{tikzpicture}
\node[obs] (w) {$w_{d,n}$};
...
\node[latent, left=of eta] (mu) {$\mu$};
\node[latent, above=of eta] (sigma) {$\Sigma$};
\edge{z}{w};
\edge{beta}{w};
\edge{eta}{z};
\edge{mu}{eta};
\edge{sigma}{eta};
\plate [inner sep=.1cm] {plate1} {(w) (z)} {$N$};
\plate [inner sep=.1cm] {plate2} {(plate1) (eta)} {$D$};
\plate [inner sep=.1cm] {plate3} {(beta)} {$K$};
\end{tikzpicture}
```
...
- The **Structural Topic Model** (Roberts, Stewart, Tingley, Airoldi, 2013) extends the correlated topic model to incorporate covariates:
  - Allow covariates $X_d$ to enter into the logistic-normal distribution governing the topic proportions $\eta_d \sim \text{Normal}(\mu_{d}, \Sigma)$  where $\mu_{d,k} = X_{d}^{\prime}\gamma_{k}$
  - Allow covariates $Y_d$ to enter into the $\beta$ parameter governing topic content $\beta_{d, k} \propto \exp(m + \kappa_k^{(t)} + \kappa_{y_d}^{(c)} + \kappa_{y_d,k}^{(i)})$

---

# Structural Topic Model

```{tikz, stm, echo=F, fig.align='center'}
\usetikzlibrary{arrows}
\usetikzlibrary{bayesnet}
\begin{tikzpicture}
\node[obs] (w) {$w_{d,n}$};
\node[latent, left=of w] (z) {$z_{d,n}$};
\node[latent, right=of w] (beta) {$\beta_{d,k}$};
\node[latent, left=of z] (eta) {$\eta_d$};
\node[latent, above=of eta] (sigma) {$\Sigma$};
\node[latent, left=of eta] (mu) {$\mu_d$};
\node[obs, below=of mu] (x) {$X_d$};
\node[obs, below=of beta] (y) {$Y_d$};
\node[latent, above=of mu] (gamma) {$\gamma$};
\node[latent, above=of beta] (kappa) {$\kappa$};
\edge{z}{w};
\edge{beta}{w};
\edge{eta}{z};
\edge{mu}{eta};
...
\plate [inner sep=.1cm] {plate1} {(w) (z)} {$N$};
\plate [inner sep=.1cm] {plate3} {(beta)} {$K$};
\plate [inner sep=.1cm] {plate2} {(plate1) (plate3) (eta) (mu) (x) (y)} {$D$};
\end{tikzpicture}
```
...
```{r, warning=F, message=F, results=F, cache=T}
...
- Common criteria for ranking the "top" words
  - **Highest probability** - Which words have the highest probability in the topic-word distribution $\beta_{k} = p(w | z = k)$
  - **Frequency-Exclusivity** (FREX) - Harmonic mean of rank by within-topic probability and rank by topic distribution given word $p(z | w = v)$
...
```{r,fig.height=6, fig.width=10, fig.align="center", warning=F, message=F}
plotQuote(unlist(findThoughts(poliBlog_fit, texts = out$meta$documents, n=3, topics=20)$docs), maxwidth=300, width=100)
```
...
```{r,fig.height=6, fig.width=10, fig.align="center", warning=F, message=F}
plotQuote(unlist(findThoughts(poliBlog_fit, texts = out$meta$documents, n=3, topics=4)$docs), maxwidth=300, width=100)
```
...
- Conventional method: **held-out log-likelihood**. For an "out-of-sample" document $\mathbf{w}_{\text{out}}$:

$$\text{Perplexity} = \exp(-\log p(\mathbf{w}_{\text{out}}| \mu, \Sigma, \beta))$$
--
...
- **Semantic coherence** 
  - Let $D(v, v^{\prime})$ define the number of times words $v$ and $v^{\prime}$ appear together in a document.
  - Semantic coherence is defined as a sum across the $W$ most probable words

$$C_k = \sum_{i=2}^W \sum_{j=1}^{i-1} \log \bigg( \frac{D(v_i, v_j) + 1}{D(v_j)}\bigg)$$
...
```{r,warning=F, message=F, echo=F, fig.height=7, fig.width=9, fig.align="center"}
plot(modelEval)
```
...
```{r,warning=F, message=F, echo=F, fig.height=7, fig.width=9, fig.align="center"}
plot(x=unlist(modelEval$results[,"K"]), y=unlist(modelEval$results[,"exclus"]), type="l", ylab="Average Exclusivity", xlab="Number of Topics", main="Exclusivity")
```
