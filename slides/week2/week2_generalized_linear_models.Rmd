...
$$h(t) = \underset{\Delta t \to 0}{\lim} \frac{P(t < T_1 \le t + \Delta t | T_1 > t)}{\Delta t} = \frac{f(t)}{S(t)} = \frac{f(t)}{1-F(t)}$$
...
- The **Kaplan-Meier** estimator for the survival function at any time $t$ involves a product over the empirical event times that precede $t$
...
```{r}
surv_model_sex <- survfit(Surv(time, status) ~ sex, data=lung)
```
