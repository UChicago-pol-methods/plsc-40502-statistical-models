...
logistic_dens
```
- Shifting the mean of $Y^*$ up by 2:

```{r, echo=F, fig.width=7, fig.height = 3, fig.align="center"}
logistic_dens <- ggplot() + xlim(-8, 8) + geom_function(fun=dlogis, args=list(location=2)) + theme_bw() + ylab("Density") + geom_area(aes(x=c(-8,-1)),stat = "function", fun = dlogis, fill = "dodgerblue", xlim = c(-8, -1),  args=list(location=2)) +  geom_area(aes(x=c(-1,2)),stat = "function", fun = dlogis, fill = "darkorange", xlim = c(-1, 2),  args=list(location=2)) + geom_area(aes(x=c(2,8)),stat = "function", fun = dlogis, fill = "yellow", xlim = c(2, 8),  args=list(location=2)) + 
  xlab("Y*") + geom_vline(xintercept=0, lty=2) + geom_vline(xintercept=-1, lty=3, col="darkgreen", lwd=1) + geom_vline(xintercept=2, lty=3, col="darkgreen", lwd=1) 
...
- We'll use our usual plug-in estimator for the Hessian at the MLE, but instead of showing that that middle term *also* equals the expected negative hessian (and therefore cancels), we'll use a different plug-in estimator for that middle using the outer product of the gradient of each observation at the MLE.
...
- Compare our bootstrap variance-covariance matrix to the sandwich estimator one
