...
- `loo` implements a leave-one-out cross-validation method that does not require re-fitting the model by using importance-sampling (Vehtari, Gelman and Gabry, 2017)
  - And interestingly, it's asymptotically equivalent to the WAIC ("Watanabe-Akaike (or widely applicable) information criterion")
--

- In order to use `loo`, we need the log-likelihood evaluated at each observation. This comes by default in canned `rstanarm` models but for custom models we need to specify to return it in the `generated quantities` block
...
model{
  beta ~ normal(0, 9); // normal prior on coefficients
