...
$$\beta | \sigma^2 \sim \text{Normal}(\beta_0, \sigma^2B^{-1}_0)$$
$$\sigma^2 \sim \text{Inverse-Gamma}(\frac{c_0}{2}, \frac{d_0}{2})$$
--

- In this case, $\beta_0$, $B_0$, $c_0$ and $d_0$ are all hyper-parameters
--

- This is referred to as the "Normal-Inverse-Gamma" model as the joint prior distribution on $\beta, \sigma^2$ is Normal-Inverse Gamma and it happens to also be the **conjugate prior** for the case with unknown $\beta$ and $\sigma^2$
  - So the posterior is known in closed form to be **Normal-Inverse-Gamma**
  - And conditional on $\sigma^2$, $\beta | \mathbf{X}, \mathbf{Y}, \sigma^2$ is also normally distributed!
--
...
\node[const, right=of beta,] (beta0) {$\beta_0$};
...
plot(y=beta_mcmc_use[,1], x=1:length(beta_mcmc_use[,1]), xlab="Iteration", ylab="beta_0", type="l")
```
...
beta_mcmc_out %>% ggplot(aes(x=Intercept)) + xlim(.01, .05) +  theme_bw() + xlab("Intercept") + geom_histogram(aes(y=after_stat(density)), bins=50)
```
...
elections_county %>% ggplot(aes(y=dem2018, x=dem2016)) + geom_point(cex=.4, alpha=.1, col="blue") + geom_abline(aes(intercept=Intercept, slope=dem2016), col="grey", alpha=.5, data=beta_mcmc_out[seq(1, nrow(beta_mcmc_out), by=400),], lwd=1) + geom_abline(aes(intercept=mean(beta_mcmc_out[seq(1, nrow(beta_mcmc_out), by=400),][,1]), slope=mean(beta_mcmc_out[seq(1, nrow(beta_mcmc_out), by=400),][,2])), col="black", alpha=.5, lwd=1) + theme_bw() + xlim(0, 1) + ylim(0,1) + xlab("2016 Democratic Presidential Vote") +ylab("2018 Democratic House Vote")
```
